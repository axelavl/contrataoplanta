"""
EmpleoEstado.cl — Scraper: TVN (Televisión Nacional de Chile)
URL: https://empleos.tvn.cl

Sitio propio con JS. Requiere Playwright.

Uso:
    python scrapers/tvn.py --dry-run --verbose
"""
...
logger = logging.getLogger("scraper.tvn")
logger.setLevel(getattr(logging, config.LOG_LEVEL))
if not logger.handlers:
    fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(name)s - %(message)s")
    sh = logging.StreamHandler()
    sh.setFormatter(fmt)
    fh = logging.FileHandler(LOG_DIR / "tvn.log", encoding="utf-8")
    fh.setFormatter(fmt)
...
FUENTE_ID = 280
INST_NOMBRE = "TVN — Televisión Nacional de Chile"
...
def obtener_html_con_js() -> str | None:
    ...
    with sync_playwright() as pw:
        ...
        try:
            logger.info("  Navegando a %s", URL_EMPLEO)
            page.goto(URL_EMPLEO, wait_until="networkidle", timeout=PAGE_LOAD_TIMEOUT)
            ...
            logger.exception("  Error Playwright TVN: %s", exc)
            return None
        finally:
            context.close()
            browser.close()
...
def parsear_html(html: str) -> list[dict]:
    ...
    for row in soup.select("table tr"):
        ...
        ofertas.append(_construir(cargo, row_text, urljoin(URL_EMPLEO, href)))
    if ofertas:
    ...
    for sel in ["[class*='job']", "[class*='position']", "[class*='vacancy']",
        ...
        if len(nodes) >= 2:
            for node in nodes:
                ...
                ofertas.append(_construir(cargo, text, urljoin(URL_EMPLEO, href)))
            if ofertas:
                return _dedup(ofertas)
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not _es_oferta(ctx):
            continue
        vistos.add(href)
        ofertas.append(_construir(texto[:300], ctx, urljoin(URL_EMPLEO, href)))
    return _dedup(ofertas)
...
def _es_oferta(texto: str) -> bool:
    if len(texto) < 10:
        return False
    t = texto.lower()
    kw = ["cargo", "vacante", "periodista", "camarógrafo", "editor", "productor",
          "profesional", "técnico", "analista", "ingeniero", "diseñador",
          "postul", "concurso", "seleccion", "contrata"]
...
def _construir(cargo: str, texto: str, url: str) -> dict:
    ...
    if not url or url == URL_EMPLEO:
        url = f"{URL_EMPLEO}#oferta-{id_ext}"
...
def _nivel(cargo: str) -> str:
    c = cargo.lower()
    if any(w in c for w in ["gerente", "director", "editor general", "jefe"]):
        return "Directivo"
    if any(w in c for w in ["técnico", "camarógrafo", "operador"]):
        return "Técnico"
    return "Profesional"
...
def ejecutar(dry_run: bool = False, verbose: bool = False) -> dict:
    ...
    inicio = time.time()
    logger.info("=" * 60)
    logger.info("INICIO - Scraper TVN")
    logger.info("=" * 60)
    ...
    try:
        ...
        if not dry_run:
            db.rollback()
        logger.exception("  Error TVN: %s", e)
        stats["errores"] += 1
        raise
    finally:
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Scraper TVN (requiere Playwright)")
